...
describe Rexpense::Resources::Organization, vcr: true do
  ...
  context 'Tags' do
    ...
    describe '#index' do
      subject { client.organizations.tags(35) }
      ...
      context "when error" do
        ...
        it "raises Rexpense::RequestError with 401 status code" do
          expect { client.organizations.tags(35) }.to raise_error(Rexpense::RequestError) do |error|
            expect(error.code).to eq(401)
          end
        end
      end
    ...
    describe '#find' do
      subject { client.organizations.find_tag(35, 64) }
    ...
    describe '#create' do
      subject { client.organizations.create_tag(35, { name: 'teste gem' }) }

      it "returns a category successfully" do
        expect(subject.class).to eq(tag_klass)
        expect(subject.name).to eq('teste gem')
      end

      context "with error" do
        it "raises Rexpense::RequestError with 422 status core" do
          expect { client.organizations.create_tag(35, { name: '' }) }.to raise_error(Rexpense::RequestError) do |error|
            expect(error.code).to eq(422)
          end
        end
      end
    ...
    describe '#update' do
      subject { client.organizations.update_tag(35, 64, { name: 'updated tag' }) }
      ...
      context "with error" do
        it "raises Rexpense::RequestError with 404 status code" do
          expect { client.organizations.update_tag(35, 88888888, {}) }.to raise_error(Rexpense::RequestError) do |error|
            expect(error.code).to eq(404)
          end
        end

        it "raises Rexpense::RequestError with 422 status core" do
          expect { client.organizations.update_tag(35, 64, { name: '' }) }.to raise_error(Rexpense::RequestError) do |error|
            expect(error.code).to eq(422)
          end
        end
      end
    ...
    describe '#destroy' do
      subject { client.organizations.destroy_tag(35, 93) }
